...
class CustomerGroupsEndpoint extends BaseExtend {
	...
	getSubGroupByHQGroupId = async (HQGroupId: number): Promise<IResponse<IResGetCustomerGroup[]>> => {
		const res = await this.request.get(`/bo/customer-group/sub-groups/${HQGroupId}`)
		return res
	}

	updateGroupAssignee = async (groupAssignee: IReqUpdateGroupAssignee): Promise<IResponse<IResCustomerGroup[]>> => {
	...
	updateGroupMaxValue = async (groupAssignee: IReqUpdateGroupMaxValue): Promise<IResponse<IResRawData[]>> => {
		const res = await this.request.put(`bo/customer-group/${groupAssignee.group_id}/update`, groupAssignee)
		return res
	}
}
