...
const calculateResponse = async (promise: Promise<Response>): Promise<IResponse<any>> => {
  let code = 200
  let message = ''
  let hasError = false
  let data = null
...
class RequestFactory {
  ...
  get: IGet = async (path, signal = null, headers = DEFAULT_AUTH_HEADER) => {
    try {
      ...
      const { data, hasError, code, message } = await apiProcessor(whenResponse, this.options)
